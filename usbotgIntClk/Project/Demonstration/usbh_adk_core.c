/**
  ...
  * version V1.0.0
  * date    2019/01/30
  * brief   Android Open Accessory implementation(Source file)
  ******************************************************************************
  */
...
/**  
* USBH_ADK_CORE_Private_Variables
*/
...
/** 
* USBH_ADK_CORE_Private_FunctionPrototypes
*/
...
/**
  * brief  USBH_ADK_Init, Initialization for ADK class.
			...
			if( USBH_Get_DevDesc(pdev , phost, USB_DEVICE_DESC_SIZE)== USBH_OK ){
				...
				if(pphost->device_prop.Dev_Desc.idVendor == USB_ACCESSORY_VENDOR_ID &&
				   (pphost->device_prop.Dev_Desc.idProduct == USB_ACCESSORY_PRODUCT_ID ||
					...
					uint16_t Psr= pphost->device_prop.Dev_Desc.idProduct;
					ADK_Machine.initstate = ADK_INIT_CONFIGURE_ANDROID;
	...
	switch (ADK_Machine.state)
	{
		...
		case ADK_SEND_DATA:
			...
			if(ADK_Machine.outSize > 0){
				...
				ADK_Machine.outSize = 0;
				ADK_Machine.state = ADK_GET_DATA; //COMMENTED BY MRIDUL
			}//ADK_Machine.state = ADK_GET_DATA; //ADDED BY MRIDUL
			break;
...
//				ADK_Machine.state = ADK_IDLE; // ADDED BY MRIDUL
//				break;
//			}
