...
extern int charge;
uint8_t next_sno[64], prev_sno[64], sno_matched;
...
void reinit_ADK(void);
//extern USBH_Status USBH_ADK_InterfaceInit ( USB_OTG_CORE_HANDLE *pdev, void *phost);
...
/** @defgroup USBH_CORE_Private_FunctionPrototypes
  * @{
  */
extern int firsttime,chargershut;
...
//    }      
  ...
  case HOST_DEV_DISCONNECTED :
    ...
    phost->gState = HOST_IDLE;
//    if(firsttime == 2 && chargershut == 0)
//			{
//      firsttime = 0;
//			reinit_ADK();
//			//charge = 0;
    if(firsttime == 1 && chargershut == 0){
      firsttime = 0;
			Delay(100);
			reinit_ADK();			
		}
...
static USBH_Status USBH_HandleEnum(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  case ENUM_IDLE:  
    /* Get Device Desc for only 1st 8 bytes : To get EP0 MaxPacketSize */
			/*From here J6 Mobile is not working*/ 
    ...
    { /* Check that Serial number string is available */    
                               ...
                               0xff) == USBH_OK)
      {
        /* User callback for Serial number string */
				/*Copy the serial number of Android device with previous */
				//memcpy (prev_sno, next_sno, sizeof(next_sno));
				memcpy (next_sno, Local_Buffer, sizeof(Local_Buffer));
				if(strstr((char*)prev_sno, (char*)next_sno))		//Replace SrNo. of the device with new one.
					sno_matched = 1;
					else sno_matched = 0;
